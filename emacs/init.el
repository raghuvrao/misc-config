...
(defun raghu--project-go-implement-project-root ()
  "Provide an implementation for the generic function `project-root'."
  (cl-defmethod project-root ((project (head go-module)))
    (cdr project)))

(defun raghu--project-find-go-module (dir)
  (when-let ((root (locate-dominating-file dir "go.mod")))
    (cons 'go-module root)))

(defun raghu--eglot-interactively-organize-imports ()
  "Provide arguments to `eglot-code-action-organize-imports' via `call-interactively'."
  (call-interactively #'eglot-code-action-organize-imports))

(defun raghu--eglot-organize-imports-before-save ()
  "Organize imports before saving the buffer."
  (add-hook 'before-save-hook #'raghu--eglot-interactively-organize-imports nil t))

(defun raghu--eglot-format-buffer-before-save ()
...
(with-eval-after-load 'go-mode
  ...
  (add-hook 'go-mode-hook #'raghu--project-go-implement-project-root)
  (add-hook 'go-mode-hook #'raghu--eglot-organize-imports-before-save)
  (add-hook 'go-mode-hook #'raghu--eglot-format-buffer-before-save))
...
(with-eval-after-load 'project
  (add-hook 'project-find-functions #'raghu--project-find-go-module))
...
(custom-set-variables
 ...
 '(package-selected-packages
   '(gnu-elpa-keyring-update lyrics markdown-mode expand-region yasnippet company eglot go-mode yaml-mode))
